    ...
    public class MainWindow : MonoBehaviour
    {
        ...
        private void CreateUI()
        {
            if (canvas == null)
            {
                ...
                #region[创建标题 和 关闭按钮]
                AddTitle(background, $"神州志西游内置修改 by:Jim97");
                ...
                elementY += 5;
                #region[添加功能按钮]
                AddH3("角色功能：", BasicScripts);
                {
                    AddButton("唐僧:布道", BasicScripts, () =>
                    {
                        Scripts.PickCardsFromDeckForSummonNPC();
                        optionToggle = false;
                        canvas.SetActive(optionToggle);
                    });
                    AddButton("八戒/悟空:冥想", BasicScripts, () =>
                    {
                        Scripts.MeditateCard();
                        optionToggle = false;
                        canvas.SetActive(optionToggle);
                    });
                    AddButton("白龙:补满灵力", BasicScripts, () =>
                    {
                        Player player = Game.Get().GetPlayer();
                        if (player is WhiteDragon)
                        {
                            RLDragonSoulStone relic = player.GetRelic<RLDragonSoulStone>();
                            relic.Count = relic.MaxCount;
                        }
                    });
                    AddButton("白龙:增加灵力上限", BasicScripts, () =>
                    {
                        Player player = Game.Get().GetPlayer();
                        if (player is WhiteDragon)
                        {
                            UIWindows.SpawnInputDialog($"您想增加多少灵力上限？", "获得", "10", (string count) =>
                            {
                                int v = count.ConvertToIntDef(10);
                                RLDragonSoulStone relic = player.GetRelic<RLDragonSoulStone>();
                                relic.MaxCount += v;
                            });
                        }
                    });
                    AddButton("白龙:学习同伴牌", BasicScripts, () =>
                    {
                        Player player = Game.Get().GetPlayer();
                        if (player is WhiteDragon dragon)
                        {
                            if (!dragon.GetDragonPrincess().CheckValidToLearnCard())
                            {
                                return;
                            }
                            BasicCallback confirm = delegate ()
                            {
                                Game.Get().EnableInput();
                            };
                            BasicCallback cancel = delegate ()
                            {
                                Game.Get().EnableInput();
                            };
                            EngineWrapper.Get().PickCompanionCardToLearn(confirm, cancel);
                            optionToggle = false;
                            canvas.SetActive(optionToggle);
                        }
                    });
                    hr(10);
                    AddButton("白龙:释放同伴牌", BasicScripts, () =>
                    {
                        Player player = Game.Get().GetPlayer();
                        if (player is WhiteDragon)
                        {
                            ...
                            EngineWrapper.Get().PickCompanionCardToUnlearn(confirm, cancel);
                            optionToggle = false;
                            canvas.SetActive(optionToggle);
                        }
                    });
                    AddButton("紫灵/青灵:断魂", BasicScripts, () =>
                    {
                        Player player = Game.Get().GetPlayer();
                        if(player is BladeSouls)
                        {
                            UIWindows.SpawnInputDialog($"您想获得多少换魂次数？", "获得", "10", (string count) =>
                            {
                                RLAmethystBlade relic = player.GetRelic<RLAmethystBlade>();
                                int v = count.ConvertToIntDef(10);
                                relic.MaxCount += v;
                                ((STBladeSoulsInternal)relic.Status).Count += v;
                            });
                        }
                    });
        ...
        public static GameObject AddButton(string Text, GameObject panel, UnityAction action)
        {
            ...
            button.GetComponent<RectTransform>().sizeDelta = new Vector2(110, 30);    // 设置按钮大小
